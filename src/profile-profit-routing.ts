...
const TARGETS = [
  {
    address: "4yWaU1QrwteHi1gixoFehknRP9a61T5PhAfM6ED3U2bs",
    label: "profit_pass_1",
    context: "Profit passthrough wallet #1 in network map. Needs full profiling.",
  },
  {
    address: "HDTncsSnBmJWNRXd641Xuh8tYjKXx1xcJq8ACuCZQz52",
    label: "profit_pass_2",
    context: "Profit passthrough wallet #2 in network map. Needs full profiling.",
  },
  {
    address: "8CvuX95RTCCBfUY8cUET8FQZusoY49yUkQxTEL2skYyq",
    label: "cold_usdc_1",
    context: "Cold USDC wallet #1 in network map. Needs full profiling.",
  },
  {
    address: "EAcUbdoiY8aCwJKdSo17fhU4uqMopW27K4oLqpstqfHe",
    label: "cold_usdc_2",
    context: "Cold USDC wallet #2 in network map. Needs full profiling.",
  },
  {
    address: "J6YUyB4P4LFfHqWxJvfXQC7ktFKgvx8rzfJFEzTNJmcT",
    label: "coinbase_deposit",
    context: "Coinbase deposit address in network map. Needs full profiling.",
  },
  {
    address: "21wG4F3ZR8gwGC47CkpD6ySBUgH9AABtYMBWFiYdTTgv",
    label: "binance_deposit",
    context: "Binance deposit address in network map. Needs full profiling.",
  },
  {
    address: "RB3dQF6TsinAUsQsvXtAyxMztMHXJ2GaZ3gdMuuHiw7",
    label: "rollbit_deposit",
    context: "Rollbit deposit address in network map. Needs full profiling.",
  },
  {
    address: "DcEYX34vm5JYHmYbCCQC141XGJektNHqhnGDUoUTdGsg",
    label: "routes_binance",
    context: "Routes to Binance wallet in network map. Needs full profiling.",
  },
];
...
async function investigate(target: typeof TARGETS[number]) {
  ...
  // Counterparties (5 cr)
  try {
    const cp = await nansenCounterparties(target.address, "2025-01-01", "2026-03-24");
...
async function main() {
  console.log("=== PROFIT ROUTING WALLET PROFILING ===");
  ...
  const outputPath = join(rawDir, "profit-routing-profiles.json");
