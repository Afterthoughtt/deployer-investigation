...
const TARGETS = [
  {
    address: "niggerd597QYedtvjQDVHZTCCGyJrwHNm2i49dkm5zS",
    label: "niggerd5",
    context: "L10 buyer #13. $916 at +3 seconds. Nansen label: 'Deployer'. Very fast, significant volume.",
  },
  {
    address: "BqP79WmkKFRytsrWCqY2ra4n5XzUxQjTf45rHy7rkgtC",
    label: "BqP79Wmk",
    context: "L10 buyer #24. $689 at +6 seconds. Unlabeled. Significant volume.",
  },
  {
    address: "7QJM8rXXUz4vTgKyQJNJyCtNLe9YuSpQoSsQUMZSRWHj",
    label: "7QJM8rXX",
    context: "L10 buyer #63. $1,075 at +10 seconds. Unlabeled. LARGEST non-deployer buy.",
  },
  {
    address: "4tMmABq7ZE1yaKRacMPewimJeUKJYzVrMf73BhPaAtcK",
    label: "4tMmABq7",
    context: "L10 buyer #114. $612 at +63 seconds. Unlabeled.",
  },
];
...
async function investigate(target: typeof TARGETS[number]) {
  ...
  if (fb && !fb.notFound && fb.funder && !KNOWN[fb.funder]) {
    ...
    try {
      ...
      if (l2Identity && !l2Identity.notFound) console.log(`L2 Identity: ${l2Identity.name} (${l2Identity.category})`);
      if (l2FundedBy && !l2FundedBy.notFound) {
...
async function main() {
  console.log("=== Profiling 4 Suspicious L10 Early Buyers ===");
  ...
  const outPath = join(rawDir, "l10-suspicious-buyers-profiles.json");
