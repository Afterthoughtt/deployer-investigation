...
- When a review artifact classifies an address that already exists in `data/network-map.json`, start by reading the canonical `verdict`, `role`, and label/source context, but treat them as **a hypothesis to verify against fresh on-chain data — not a starting truth.** The registry has been wrong before (see the incident list under "Primitive Integrity"), and a stale prior carried forward silently becomes a false conclusion. Surface the registry prior explicitly in the artifact, then grade from fresh evidence. If fresh evidence agrees, the grade is confirmed; if it disagrees, mark `proposed_registry_change: true` and attach the evidence.
...
- **SPL transfer from the deploy wallet ranks above an open-market buy.** Bundle recipients (6 per launch, per behavioral profile) are explicitly chosen by the deployer; open-market buys within the first seconds are public and can be hit by any pre-whitelisted sniper. A wallet that received a prior-launch token as a direct inbound SPL transfer from the deploy wallet is near-certain deployer control on a single-launch hit. Always parse the first wallet-involving transaction for a matched CA before grading — instruction type distinguishes allocation from a buy.
- **Prior-launch participation is a first-class grading input and computed on-demand, not pre-annotated.** The 10 prior-launch CAs and `created_utc` values live in `data/launch-history.json`. For any wallet, call `getTokenAccountsByOwner` (10 credits) and filter the returned mints against those 10 CAs; for any match, pull the earliest signature touching that ATA and compute the delta against `created_utc` to derive buy-timing. The `early_buyers` arrays in `data/launch-details.json` are historical snapshots — current state lives on-chain. Closed-ATA wallets miss the cheap path and require Wallet API history (100 credits, Pass C) to recover participation.
