...
for (const [k, v] of Object.entries(networkMap.infrastructure || {})) { const a = (v as any).address; if (a) KNOWN[a] = k; }
for (const [k, v] of Object.entries(networkMap.bundle_wallets || {})) KNOWN[v as string] = k;
for (const [k, v] of Object.entries(networkMap.profit_routing || {})) KNOWN[v as string] = k;
...
async function heliusHistory(address: string, limit = 100) {
  const res = await fetch(`${HELIUS_API}/v1/wallet/${address}/history?api-key=${HELIUS_KEY}&limit=${limit}&tokenAccounts=balanceChanged`);
...
const TARGETS = [
  {
    address: "9exPdTUVTCz9EKvZjXkKJSTJ5fZzJuwJHnFptrUFHFNH",
    label: "9exPdTUV",
    context: "$13.5K in PURE OUTFLOWS from 5 bundles + Hub. Never sends back. Profit extraction suspect.",
  },
  {
    address: "BR1HiYtcYv5L9oB9aydMS9G9nYSjVZcMB51AxBFUkQ8T",
    label: "BR1HiYtc",
    context: "$4.1K outflow from 3 bundles + Hub. Pure outflow pattern.",
  },
  {
    address: "4916NkdubkfRyHkxkCR7rpVGz5dvzVdK161mg4jXDwRh",
    label: "4916Nkdu",
    context: "$870 outflow from 2 bundles. Pure outflow.",
  },
  {
    address: "D1XcKeSSiZNX9bE56meS8hcR3ZEeoBU3GAejDdm3sTVQ",
    label: "D1XcKeSS",
    context: "$1.5K from 2 bundles. $710 mostly inflow from bundle_1.",
  },
];
...
async function investigate(target: typeof TARGETS[number]) {
  ...
  try {
    ...
    if ((fb as any).notFound) {
      ...
      if (KNOWN[funder]) result.network_connections.push(`First funder: ${KNOWN[funder]} (${funder})`);
    }
  } catch (e: any) {
  ...
  // Signatures (10 cr) — check activity window
  try {
  ...
  // Transfers (100 cr) — full transfer history
  try {
  ...
  // History (100 cr) — balance changes per tx
  try {
    const hist = await heliusHistory(target.address, 50);
    const data = (hist as any)?.data;
    result.helius.history = hist;
    if (Array.isArray(data) && data.length > 0) {
      console.log(`[History] ${data.length} txs with balance changes:`);
      for (const h of data.slice(0, 20)) {
        const ts = h.timestamp ? new Date(h.timestamp * 1000).toISOString().slice(0, 19) : "?";
        const changes = (h.balanceChanges || [])
          .filter((c: any) => Math.abs(c.amount) > 0.001)
          .map((c: any) => `${c.amount > 0 ? "+" : ""}${Number(c.amount).toFixed(4)} ${c.symbol || "?"}`)
          .join(", ");
        if (changes) console.log(`  ${ts} | ${changes}`);
      }
    } else {
      console.log(`[History] No balance changes`);
    }
  } catch (e: any) {
    console.log(`[History] ERROR: ${e.message}`);
    result.errors.push(`history: ${e.message}`);
  }
  ...
  // Counterparties (5 cr) — wide date range
  try {
    const cp = await nansenCounterparties(target.address, "2025-01-01", "2026-03-21");
    result.nansen.counterparties = cp;
    const data = (cp as any)?.data;
    if (Array.isArray(data) && data.length > 0) {
      console.log(`[Counterparties] ${data.length} counterparties:`);
      for (const c of data) {
        ...
        } else if (!label?.includes("Pump.fun") && !label?.includes("Bonding Curve") && !label?.includes("Liquidity Pool")) {
...
// === LAYER 2: Investigate funders of L1 targets ===
async function investigateFunder(funderAddr: string, parentLabel: string) {
  ...
  // Transfers (100 cr)
  try {
    const transfers = await heliusTransfers(funderAddr, 50);
    const data = (transfers as any)?.data;
    result.helius.transfers = transfers;
    if (Array.isArray(data) && data.length > 0) {
      console.log(`[Transfers] ${data.length} transfers:`);
      for (const t of data.slice(0, 20)) {
        ...
        const ts = t.timestamp ? new Date(t.timestamp * 1000).toISOString().slice(0, 19) : "?";
        console.log(`  ${ts} ${dir} ${cp.slice(0, 16)}... ${cpTag} | ${amt} ${sym}`);
        if (KNOWN[cp]) result.network_connections.push(`Transfer ${t.direction}: ${KNOWN[cp]}`);
      }
    } else { console.log(`[Transfers] None`); }
  } catch (e: any) { result.errors.push(`transfers: ${e.message}`); }

  // Nansen related (1 cr)
  await sleep(2000);
  try {
    const rw = await nansenRelatedWallets(funderAddr);
    result.nansen.related_wallets = rw;
    const data = (rw as any)?.data;
    if (Array.isArray(data) && data.length > 0) {
      console.log(`[Nansen Related] ${data.length}:`);
      for (const r of data) {
        console.log(`  ${r.address?.slice(0, 16)}... ${tag(r.address, r.address_label)} — ${r.relation}`);
        if (KNOWN[r.address]) result.network_connections.push(`Related: ${KNOWN[r.address]} (${r.relation})`);
      }
    } else { console.log(`[Nansen Related] None`); }
  } catch (e: any) { result.errors.push(`nansen_related: ${e.message}`); }
  ...
  result.network_connections = [...new Set(result.network_connections)];
  console.log(`\nL2 Network connections: ${result.network_connections.length}`);
  for (const c of result.network_connections) console.log(`  - ${c}`);
...
async function main() {
  console.log("=== DEEP INVESTIGATION: Bundle Unknown Wallets ===");
  console.log(`Targets: ${TARGETS.length}`);
  console.log(`Credit estimate per target: ~411 (balance=1, identity=100, funded-by=100, sigs=10, transfers=100, history=100) + Nansen 6 (related=1, counterparties=5)`);
  console.log(`Total: ~${TARGETS.length * 417} Helius + ${TARGETS.length * 6} Nansen + Arkham free\n`);
  ...
  for (const t of TARGETS) {
    ...
    // Layer 2: If we found a funder, investigate the funder too
    ...
    if (funder && !KNOWN[funder] && !(result.helius.funded_by as any).notFound) {
      ...
      // Layer 3: If the funder's funder is also unknown, go one more level
      const funder2 = funderResult.helius?.funded_by?.funder;
      if (funder2 && !KNOWN[funder2] && !(funderResult.helius.funded_by as any).notFound) {
        await sleep(2000);
        const f2Result = await investigateFunder(funder2, `L2-funder-of-${t.label}`);
        f2Result.layer = 3;
        funderResults.push(f2Result);
      }
    }
  ...
  const outputPath = join(resultsDir, "bundle-unknowns-deep.json");
